...
vector<Row> rows;
sortRow(){
sort(rows.begin(),rows.end(),[=](Row a,Row b)->{return(a<b);});
}
void input(FILE *fp){//此处用的是文件流,返回给自己
if((fp=fopen(filename,"r"))==NULL){throw ("ERROR");}
string line;
else while(getline(fp,line)){//这里这样子不知道能不能运行qwq
string tmp;
Text t;
